...
let dollar_test = Dollar 1.220


let dollar_to_euro e =
  match e with
...
let rec intbool_reverse intbool_list = 
  let rec aux acc intbool_list =
    match intbool_list with
    | Empty -> acc
    | Int (i, ibl) -> aux (Int (i, acc)) ibl
    | Bool (b, ibl) -> aux (Bool (b, acc)) ibl
  in
...
let intbool_separate ib_list = 
  let rec aux acci accb ib_list = 
    match ib_list with
    | Empty -> (intbool_reverse acci, intbool_reverse accb)
    | Int(i, ibl) -> aux (Int (i, acci)) accb ibl
    | Bool(b, ibl) -> aux acci (Bool (b, accb)) ibl
  in
  aux Empty Empty ib_list
(*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*]
...
let count_magic wizlist = 
  let rec aux counter wizlist = 
    match wizlist with
    | [] -> counter
    | {name; status} :: restlist -> (
        match status with
        | Newbie -> aux counter restlist
        | Student (magic, _) -> aux (update counter magic) restlist
        | Employed (magic, _) -> aux (update counter magic) restlist
      )
  in
  aux {fire = 0; frost = 0; arcane = 0} wizlist
(*----------------------------------------------------------------------------*]
