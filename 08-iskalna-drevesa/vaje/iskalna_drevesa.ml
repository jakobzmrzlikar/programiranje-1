...
let rec mirror tree = 
  match tree with
  | Empty -> Empty
  | Node(ltree, x, rtree) -> Node(mirror rtree, x, mirror ltree)
...
let rec height tree = 
  match tree with
  | Empty -> 0
  | Node(ltree, x, rtree) -> 1 + max (height ltree) (height rtree)

let rec size tree =
  match tree with
  | Empty -> 0
  | Node(ltree, x, rtree) -> 1 + size ltree + size rtree
(*----------------------------------------------------------------------------*]
...
let rec map_tree f tree =
  match tree with
  | Empty -> Empty
  | Node(ltree, x, rtree) -> Node(map_tree f ltree, f x, map_tree f rtree) 
(*----------------------------------------------------------------------------*]
...
let rec list_of_tree tree =
  match tree with
  | Empty -> []
  | Node(ltree, x, rtree) -> (list_of_tree ltree) @ [x] @ (list_of_tree rtree)
(*----------------------------------------------------------------------------*]
...
let is_bst tree =
  let is_sorted lst =
    let rec aux prev lst =
      match lst with
      | [] -> true
      | x :: xs -> if (prev <= x) then (aux x xs) else false
    in aux 0 lst
  in
...
let inside_bounds low_bound x high_bound =
...
(* za elitiste funkcijskega programiranja *)

let cmp_bind cmp x = function
  | None -> true
  | Some y -> cmp x y

let inside_bounds l x h = cmp_bind (<) x h && cmp_bind (>) x l


let is_bst tree =
  let rec aux low_bound high_bound = function
    | Empty -> true
    | Node(ltree, x, rtree) ->
      let this_ok = inside_bounds low_bound x high_bound in
      let left_ok = aux low_bound (Some x) ltree in
      let right_ok = aux (Some x) high_bound rtree in
...
let rec insert bst x =
  ...
  | Node(ltree, root, rtree) -> 
    if x < root then Node(insert ltree x, root, rtree)
    else Node(ltree, root, insert rtree x)

let rec member bst x =
  match bst with
  | Empty -> false
  | Node(ltree, root, rtree) ->
    if (x = root) then true
    else member ltree x && member rtree x 
(*----------------------------------------------------------------------------*]
